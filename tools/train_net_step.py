...
from datasets.roidb_hico import combined_roidb_for_training
from roi_data.loader import RoiDataLoader_HICO, MinibatchSampler, BatchSampler, collate_minibatch
...
def parse_args():
    ...
    parser.add_argument('--fix_FPN', action='store_true')
...
def main():
    ...
    if args.dataset == 'vcoco_trainval':
        cfg.TRAIN.DATASETS = ('vcoco_trainval',)
        cfg.MODEL.NUM_CLASSES = 81
    elif args.dataset == 'hico_trainval':
        cfg.TRAIN.DATASETS = ('hico_trainval',)
        cfg.MODEL.NUM_CLASSES = 81
    else:
    ...
    if args.copy:
        cfg.TRAIN.COPY = True
    if args.cluster:
        cfg.TRAIN.CLUSTER = True
    if args.score:
        cfg.TRAIN.SCORE = True
    if args.debug:
        cfg.DEBUG = True
    if args.mask:
        cfg.MASK = True
    ...
    cfg.HICO.TRIPLETS_NUM_PER_IM = args.triplets_num_per_im
    print('triplets_num_per_im: ', cfg.HICO.TRIPLETS_NUM_PER_IM)
    ...
    print(cfg.NUM_GPUS, cfg.TRAIN.IMS_PER_BATCH,  original_batch_size) # 8 2 16
    ...
    # print(roidb[0].keys())
    # ['image_id', 'dataset', 'image', 'flipped', 'precomp_boxes', 'precomp_score', 'precomp_cate', 'precomp_keypoints',
    #  'precomp_prob_mask', 'width', 'height', 'gt_human_boxes', 'gt_object_boxes', 'gt_hoi_ids', 'gt_hoi_vec',
    #  'gt_verb_vec', 'gt_obj_cat', 'need_crop']
    ...
    ### Model ###
    from modeling.model_builder import Generalized_RCNN as RCNN
    maskRCNN = RCNN()
    ...
    for key, value in maskRCNN.named_parameters():
        # fix  better
        if "Box_Outs" in key or "Box_Head" in key or \
                "HOI_Head.box_head" in key:
            value.requires_grad = False

        if args.fix_FPN:
            if "HOI_Head" not in key:
                value.requires_grad = False
