...
from datasets.json_dataset_hico import JsonDataset
...
def test_net_on_dataset(
    ...
    if multi_gpu:
        ...
        # print(proposal_file)
        test_net(args, dataset_name, proposal_file, output_dir, gpu_id=gpu_id)
    test_timer.toc()
...
def test_net(
    ...
    print(start_ind, end_ind, total_num_images)
    # 0 9546 9546
    ...
    for i, entry in enumerate(roidb):
        ...
        hoi_res_i, loss_i = im_detect_all_precomp_box(model, im, entry)

        global_id = entry['image_id']
        pred_hois.create_group(global_id)
        pred_hois[global_id].create_dataset(
            'human_obj_boxes_scores_hoi_idx',
            data=hoi_res_i)
        print(timers['hoi'].toc())
...
def initialize_model_from_cfg(args, gpu_id=0):
    ...
    if args.load_ckpt:
        ...
        net_utils.load_ckpt_test(model, checkpoint['model'])
...
def get_roidb_and_dataset(dataset_name, proposal_file, ind_range):
    ...
    if 'hico' in dataset_name:
        ...
        dataset = JsonDataset_HCVRD(dataset_name)
    if cfg.TEST.PRECOMPUTED_PROPOSALS:
