...
def im_detect_all(model, im, box_proposals=None, timers=None, entry=None):
    ...
    if cfg.MODEL.VCOCO_ON:
        if cfg.HICO.KEYPOINTS_ON:
            # ipdb.set_trace()
            vcoco_heatmaps, vcoco_heatmaps_np = im_detect_keypoints_vcoco(model, im_scale[0], cls_boxes[1][:, :4],
                                                                          blob_conv)
...
def im_detect_all_precomp_box(model, im, entry=None):
    """Process the outputs of model for testing
    Args:
      model: the network module
      im_data: Pytorch variable. Input batch to the model.
    ...
    im_info = np.array([im.shape[:2] + (im_scale[0],)])
    scores, boxes, cates, obj_mask = im_detect_bbox_precomp_box(entry)

    hoi_res, loss = im_detect_hoi_union(model, boxes, scores, cates, obj_mask,
                                        im_info, blob_conv, entry)
    return hoi_res, loss
...
def im_detect_bbox_precomp_box(entry):
    """Prepare the bbox for testing"""
    # box in origin image
    pred_boxes = entry['precomp_boxes']
    scores = entry['precomp_score']
    cates = entry['precomp_cate'].astype(np.int32)
    if "precomp_prob_mask" in entry:
        obj_mask = entry['precomp_prob_mask']
    else:
        obj_mask = None

    return scores, pred_boxes, cates, obj_mask
...
def im_detect_hoi(model, boxes, scores, human_count, im_info, blob_conv, entry=None, vcoco_heatmaps=None):
    ...
    if human_index.size > 0:
        ...
        hoi_res = dict(
            agents=np.full((1, 4 + cfg.HICO.NUM_ACTION_CLASSES), np.nan),
            roles=np.full((1, 5 * cfg.HICO.NUM_ACTION_CLASSES, cfg.HICO.NUM_TARGET_OBJECT_TYPES), np.nan),
        )
...
def hoi_res_gather(hoi_blob, im_scale, entry=None):
    ...
    num_action_classes = cfg.HICO.NUM_ACTION_CLASSES
    num_target_object_types = cfg.HICO.NUM_TARGET_OBJECT_TYPES
    ...
    action_mask = np.array(cfg.HICO.ACTION_MASK)
    ...
    if hoi_blob['target_object_index'].size > 0:
        ...
        top_k_value = triplet_action_score.flatten()[
            np.argpartition(triplet_action_score, -cfg.HICO.KEEP_TOP_NUM, axis=None)[-cfg.HICO.KEEP_TOP_NUM]]
    ...
    roles = np.stack([roles[:, :, i, :].reshape(-1, num_action_classes * 5) for i in range(num_target_object_types)],
                     axis=-1)
...
def target_localization(boxes, human_index, object_index, target_location):
    ...
    compatibility = np.exp(-compatibility / (2 * cfg.HICO.TARGET_SIGMA ** 2))
...
def generate_action_mat(gt_role_id):
    ...
    '''
    mat = np.zeros((gt_role_id.shape[0], gt_role_id.shape[0], cfg.HICO.NUM_ACTION_CLASSES, gt_role_id.shape[-1]),
                   dtype=np.float32)
...
def _compute_action_targets(person_rois, gt_boxes, role_ids):
    ...
    return human_action_targets.reshape(-1, cfg.HICO.NUM_ACTION_CLASSES * 2 * 4), \
           action_target_weights.reshape(-1, cfg.HICO.NUM_ACTION_CLASSES * 2 * 4)


# ------------------------------- HOI union ------------------------------------
def im_detect_hoi_union(model, boxes, scores, cates, obj_mask,
                        im_info, blob_conv, entry=None):
    ...
    keep_human_inds = np.where(human_scores >= cfg.HICO.TEST_HUMAN_SCORE_THRESH)[0]
    if len(keep_human_inds) == 0:
        keep_human_inds = np.arange(len(human_inds))
    human_boxes = human_boxes[keep_human_inds]
    human_scores = human_scores[keep_human_inds]

    keep_target_inds = np.where(scores >= cfg.HICO.TEST_TARGET_OBJECT_SCORE_THRESH)[0]
    if len(keep_target_inds) == 0:
        keep_target_inds = np.arange(len(scaled_boxes))
    target_boxes = scaled_boxes[keep_target_inds]
    target_scores = scores[keep_target_inds]
    target_classes = cates[keep_target_inds]
    target_mask = obj_mask[keep_target_inds]
    ...
    object_classes_index = np.array([int(id) - 1 for id in target_classes])
    ...
    hoi_blob_in['object_classes_index'] = object_classes_index
    hoi_blob_in['object_mask'] = target_mask
    ...
    if human_boxes.size > 0:
        ...
        assert not (human_keypoints == 0).all()

        union_kps, part_boxes, flag = get_pred_keypoints(human_boxes, human_keypoints,
                                                         interaction_human_inds, im_scale)

        vcoco_heatmaps, union_mask, rescale_kps = generate_joints_heatmap(union_kps, union_boxes[:, 1:] / im_scale,
                                                                          human_boxes[interaction_human_inds,
                                                                          1:] / im_scale,
                                                                          target_boxes[interaction_object_inds,
                                                                          1:] / im_scale)
        ...
        hoi_blob_out = model.module.hoi_net(blob_conv, hoi_blob_in)
        human_obj_boxes_scores = hoi_union_res_gather(hoi_blob_out, im_scale)
    else:
        human_obj_boxes_scores = np.empty([0, 12])
    return human_obj_boxes_scores, None


def hoi_union_res_gather(hoi_blob, im_scale):
    '''
    Convert predicted score and location to triplets
    :param hoi_blob:
    :param im_scale:
    ...
    human_boxes = human_boxes[hoi_blob['interaction_human_inds']]
    object_boxes = object_boxes[hoi_blob['interaction_object_inds']]
    human_score = human_score[hoi_blob['interaction_human_inds']]
    object_score = object_score[hoi_blob['interaction_object_inds']]
    object_class_indexes = hoi_blob['object_classes_index'][hoi_blob['interaction_object_inds']]
    cor_mat = np.load('data/corre_hico.npy')
    cor_mat_ = cor_mat[object_class_indexes]

    assert len(cor_mat_) == len(human_boxes) == len(object_boxes) == \
           len(human_score) == len(object_score)

    interaction_action_score = interaction_action_score * cor_mat_ * \
                               human_score[:, np.newaxis] * \
                               object_score[:, np.newaxis]

    interaction_action_score1 = interaction_action_score1 * cor_mat_ * \
                                human_score[:, np.newaxis] * \
                                object_score[:, np.newaxis]

    select_indexes = np.where((interaction_action_score > 0) &
                              (interaction_action_score1 > 0))

    pair_index = select_indexes[0]
    score_index = select_indexes[1]
    sel_num = len(score_index)

    human_obj_boxes_scores = np.zeros([sel_num, 12])
    human_obj_boxes_scores[np.arange(sel_num)] = \
        np.concatenate([human_boxes[pair_index],
                        object_boxes[pair_index],
                        interaction_action_score[pair_index, score_index][:, np.newaxis],
                        interaction_action_score1[pair_index, score_index][:, np.newaxis],
                        object_class_indexes[pair_index][:, np.newaxis],
                        score_index[:, np.newaxis]], 1)

    # print(human_obj_boxes_scores)
    # assert False
    return human_obj_boxes_scores


def generate_triplets(human_boxes, object_boxes, return_sp=False):
    ...
    union_boxes, sp = box_utils.get_union_box_sp(human_boxes[human_inds][:, 1:],
    ...
    if return_sp:
        return human_inds, object_inds, union_boxes, spatial_info, sp
...
def test_det_bbox_gt_action(hoi_blob_in, entry, im_info):
    ...
    # convert to 24-class
    interaction_action_mask = np.array(cfg.HICO.ACTION_MASK).T
...
def get_gt_labels(entry, human_boxes, obj_boxes, interaction_pair_num, im_scale):
    ...
    interaction_labels = np.zeros((interaction_pair_num, 117))
    ...
    if len(human_valid) > 0 and len(obj_valid) > 0:
        ...
        valid_interaction_affinity = np.any(valid_interaction_labels.reshape(valid_interaction_labels.shape[0], -1) > 0,
                                            1)
