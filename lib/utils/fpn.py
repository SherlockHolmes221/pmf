...
def add_multilevel_roi_blobs(
    ...
    lvl_max: the coarsest (lowest resolution) FPN level (e.g., 6)
    """
...
def distribute_rois_over_fpn_levels(blobs, rois_blob_name):
    ...
    target_lvls = map_rois_to_fpn_levels(
        blobs[rois_blob_name][:, 1:5], lvl_min, lvl_max
    )
    # Add per FPN level roi blobs named like: <rois_blob_name>_fpn<lvl>
    add_multilevel_roi_blobs(
