...
def relative_pos(human_box, object_box):
    pass
...
def aspect_ratio(boxes, aspect_ratio):
    ...
    boxes_ar = boxes.copy()
    boxes_ar[:, 0::4] = aspect_ratiso * boxes[:, 0::4]
...
def get_union_box(human_rois, target_object_rois):
    assert human_rois.shape[0] == target_object_rois.shape[0]
    assert human_rois.shape[1] == target_object_rois.shape[1] == 4
    return np.concatenate((np.minimum(human_rois[:, :2], target_object_rois[:, :2]),
                           np.maximum(human_rois[:, 2:], target_object_rois[:, 2:])), axis=1)


def get_union_box_sp(human_rois, object_rois):
    assert human_rois.shape[0] == object_rois.shape[0]
    assert human_rois.shape[1] == object_rois.shape[1] == 4
    union_boxes = np.concatenate((np.minimum(human_rois[:, :2], object_rois[:, :2]),
                                  np.maximum(human_rois[:, 2:], object_rois[:, 2:])), axis=1)
    # print(human_rois.shape)  # (32, 4)
    # print(object_rois.shape)  # (32, 4)
    # print(union_boxes.shape)  # (32, 4)

    union_w, union_h = union_boxes[:, 2] - union_boxes[:, 0], union_boxes[:, 3] - union_boxes[:, 1]

    sp = np.concatenate([((human_rois[:, 0] - union_boxes[:, 0]) / union_w).reshape(-1, 1),
                         ((human_rois[:, 1] - union_boxes[:, 1]) / union_h).reshape(-1, 1),
                         ((human_rois[:, 2] - union_boxes[:, 0]) / union_w).reshape(-1, 1),
                         ((human_rois[:, 3] - union_boxes[:, 1]) / union_h).reshape(-1, 1),
                         ((object_rois[:, 0] - union_boxes[:, 0]) / union_w).reshape(-1, 1),
                         ((object_rois[:, 1] - union_boxes[:, 1]) / union_h).reshape(-1, 1),
                         ((object_rois[:, 2] - union_boxes[:, 0]) / union_w).reshape(-1, 1),
                         ((object_rois[:, 3] - union_boxes[:, 1]) / union_h).reshape(-1, 1)], axis=1)
    # print(sp.shape)
    # print(union_boxes[3])
    # print(human_rois[3])
    # print(object_rois[3])
