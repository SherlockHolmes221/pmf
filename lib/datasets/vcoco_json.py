...
class VCOCO(object):
    ...
    def do_eval(self, dets, vcocodb, detections_file=None, ovr_thresh=0.5, plot_save_path=None):
        ...
        role_ap = self._do_role_eval(vcocodb, dets, ovr_thresh=ovr_thresh, eval_type='scenario_1',
    ...
    def _save_error(self, vcocodb, dets, ovr_thresh=0.5, eval_type='scenario_1', save_file=None):
        ...
        for i in range(len(vcocodb)):
            ...
            gt_actions = vcocodb[i]['gt_actions'][
                gt_h_inds]  # Nx26 binary array indicating the actions performed by this person
            ...
            for aid in range(self.num_actions):
                ...
                for rid in range(len(self.roles[aid]) - 1):  # rid = 0, instr; rid = 1, obj
                    ...
                    # get gt roles for action and role
                    gt_role_inds = vcocodb[i]['gt_role_id'][
                        gt_h_inds, aid, rid]  # Nx1 index of the object among all detected objects related to this action. -1 means missing object.
                    ...
                    for j in idx:  # in this image, this action with highest action score
                        ...
                        if ovmax < 0.5:  # person mis-loc
                            ...
                            if is_true_action == True and ov_max_j_objs == -1.0:  # occlusion
                                ...
                                res.append(
                                    np.concatenate([pred_box, role_boxes[j, :], [aid, rid, agent_scores[j]], [5]]))
                                ...
                                res.append(
                                    np.concatenate([pred_box, role_boxes[j, :], [aid, rid, agent_scores[j]], [2]]))
                        ...
                        #     res.append(np.concatenate([pred_box, role_boxes[j, :], [aid, rid, agent_scores[j]], [4]]))
                            ...
                            if not covered[jmax]:
                                tp += 1
                                res.append(
                                    np.concatenate([pred_box, role_boxes[j, :], [aid, rid, agent_scores[j]], [0]]))
                                covered[jmax] = True
                            else:
                                fp6 += 1
                                res.append(np.concatenate([pred_box, role_boxes[j, :], [aid, rid, agent_scores[j]],
                                                           [6]]))  # triplet duplicate
