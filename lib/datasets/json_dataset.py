...
class JsonDataset(object):
    ...
    def get_roidb(
        ...
        if self.vcoco and cfg.VCOCO.USE_PRECOMP_BOX:
            ...
            precomp_bbox_kps_filepath = os.path.join(self.cache_path, 'addPredPose',
                                                     self.name + '_precomp_boxes_keypoints_ican2.json')
    ...
    def _prep_roidb_entry(self, entry):
        ...
        if self.vcoco:
            ...
            entry['gt_role_id'] = np.empty((0, self.VCOCO.num_actions, cfg.VCOCO.NUM_TARGET_OBJECT_TYPES),
                                           dtype=np.int32)
    ...
    if cfg.VCOCO.USE_PRECOMP_BOX:
        ...
        for i in range(len(roidb)):
            ...
            if not cfg.TRAIN.USE_GT:
                ...
                # print(' use gt')
                box_list.append(np.concatenate((roidb[i]['boxes_aug'],
                                                roidb[i]['precomp_boxes']), axis=0))
    else:
        box_list = []
        for i in range(len(roidb)):
            inv_im_scale = 1. / scales[i]
