...
from .json_dataset_hico import JsonDataset
...
def combined_roidb_for_training(dataset_names, proposal_files):
    ...
    def get_roidb(dataset_name, proposal_file):
        if "hcvrd" in dataset_name:
            ds = JsonDataset_HCVRD(dataset_name)
        else:
    ...
    roidbs = [get_roidb(*args) for args in
              zip(dataset_names, proposal_files)]
    roidb = roidbs[0]
...
def extend_with_flipped_entries(roidb, dataset):
    ...
    for entry in roidb:
        # print(entry.keys())
        # dict_keys(
        #     ['image_id', 'dataset', 'image', 'flipped', 'has_visible_keypoints', 'boxes', 'gt_classes', 'gt_overlaps',
        #      'is_crowd', 'box_to_gt_ind_map', 'gt_keypoints', 'precomp_boxes', 'precomp_score', 'precomp_cate',
        #      'precomp_keypoints', 'width', 'height', 'gt_human_boxes', 'gt_object_boxes', 'gt_hoi_ids'])
        entry['flipped'] = False
        width = entry['width']
        human_boxes = entry['gt_human_boxes'].copy()
        object_boxes = entry['gt_object_boxes'].copy()
        precomp_boxes = entry['precomp_boxes'].copy()

        h_oldx1 = human_boxes[:, 0].copy()
        h_oldx2 = human_boxes[:, 2].copy()
        human_boxes[:, 0] = width - h_oldx2 - 1
        human_boxes[:, 2] = width - h_oldx1 - 1

        o_oldx1 = object_boxes[:, 0].copy()
        o_oldx2 = object_boxes[:, 2].copy()
        object_boxes[:, 0] = width - o_oldx2 - 1
        object_boxes[:, 2] = width - o_oldx1 - 1
        ...
        assert (human_boxes[:, 2] >= human_boxes[:, 0]).all()
        assert (object_boxes[:, 2] >= object_boxes[:, 0]).all()
        assert (precomp_boxes[:, 2] >= precomp_boxes[:, 0]).all()

        flipped_entry = {}
        dont_copy = ('gt_human_boxes', 'gt_object_boxes',
                     'flipped', 'precomp_boxes', "precomp_keypoints")

        for k, v in entry.items():
            if k not in dont_copy:
                flipped_entry[k] = v

        flipped_entry['gt_human_boxes'] = human_boxes
        flipped_entry['gt_object_boxes'] = object_boxes
