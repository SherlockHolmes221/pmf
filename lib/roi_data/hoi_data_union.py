...
def _sample_human_union_boxes(rois, rois_to_gt_ind, roidb, im_info, batch_idx, keep_rois_inds):
    ...
    gt_union_heatmap, union_mask, rescale_kps = \
        generate_joints_heatmap(union_gt_kps,
                                triplets_info['union_boxes'][:, 1:] / float(
                                    im_info[2]), \
                                rois[rois_human_inds[
                                    triplets_info['human_inds']]][:,
                                1:] / float(im_info[2]), \
                                rois[rois_object_inds[
                                    triplets_info['object_inds']]][:,
                                1:] / float(im_info[2]), \
                                gaussian_kernel=(cfg.VCOCO.HEATMAP_KERNEL_SIZE,
                                                 cfg.VCOCO.HEATMAP_KERNEL_SIZE))
