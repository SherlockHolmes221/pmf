...
class RoiDataLoader(data.Dataset):
    def __init__(self, roidb, num_classes, training=True):
        ...
        # print(self.DATA_SIZE, self._num_classes) # 75264 81
...
class RoiDataLoader_HICO(data.Dataset):
    def __init__(self, roidb, num_classes, training=True):
        self._roidb = roidb
        self._num_classes = num_classes
        self.training = training
    ...
    def __getitem__(self, index_tuple):
        ...
        # print(ratio)
        single_db = [self._roidb[index]]
        # print(single_db[0].keys())
        # dict_keys(['image_id', 'dataset', 'image', 'precomp_score', 'precomp_cate', 'width', 'height', 'gt_hoi_ids',
        #            'gt_hoi_vec', 'gt_verb_vec', 'gt_human_boxes', 'gt_object_boxes', 'precomp_boxes',
        #            'precomp_keypoints', 'flipped', 'need_crop'])

        ## _add_proposals(xxx)
        blobs, valid = get_minibatch(single_db)
        # print(blobs['roidb'][0].keys())
        # dict_keys(['image_id', 'flipped', 'precomp_boxes', 'precomp_keypoints',
        # 'precomp_score', 'gt_object_boxes',
        # 'gt_hoi_ids', 'gt_verb_vec', 'gt_hoi_vec'])
        ...
        #     valid_inds = np.nonzero(~ invalid)[0]
        #     if len(valid_inds) < len(boxes):
        #         for key in ['boxes', 'precomp_keypoints', 'gt_classes',
        #                     'seg_areas', 'gt_overlaps', 'is_crowd',
        #                     'box_to_gt_ind_map', 'gt_keypoints',
        #                     'gt_actions', 'gt_role_id']:
        #             if key in entry:
        #                 entry[key] = entry[key][valid_inds]
        #         entry['segms'] = [entry['segms'][ind] for ind in valid_inds]
...
class MinibatchSampler(torch_sampler.Sampler):
    def __init__(self, ratio_list, ratio_index):
        ...
        if cfg.TRAIN.ASPECT_GROUPING:
            ...
            # [0.3375     0.3375     0.3375     ... 3.69942197 3.69942197 3.69942197]
            # print(self.ratio_list_minibatch)
    ...
    def __iter__(self):
        if cfg.TRAIN.ASPECT_GROUPING:
            ...
            # print(ratio_list_minibatch)
            # print(len(ratio_list_minibatch))
            # [0.6640625  0.6640625  0.6640625... 1.39433551 1.39433551 1.39433551]
            # 19092
        else:
...
def collate_minibatch(list_of_blobs):
    ...
    # it can't be batch into a tensor.
